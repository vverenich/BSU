...
void getParticularNodes(Node* node,  std::vector<Node*>& v) {
	if (node->left == nullptr && node->right == nullptr) {
		return;//can be unsafe
	}
		if (node->right == nullptr && node->left != nullptr) {
		...
		if (node->left != nullptr) {
			getParticularNodes(node->left, v);
		}
		if (node->right != nullptr) {
			getParticularNodes(node->right,v);
		}
...
int main() {
	...
	getParticularNodes(root, particularNodes);

	for (const auto a : particularNodes) {
		std::cout << a->key << " ";
	}
