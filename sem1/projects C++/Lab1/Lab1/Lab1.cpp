...
double Kolmogorov(double* a, int n)
{
	std::sort(a, a + n);
	int i = 0;
	double D = 0;
...
double Pirson(double* a, int n, int k)
{
	std::sort(a, a + n);
	int i = 0;
	int count = 0, j = 0;
	...
	for (j = 1; j <= k; j++)
	{
		...
		//cout<<"Check"<<((double)j)/m*n;
		xi2 += (count - ((double)n) / k) / (((double)n) * k);
		//xi2 += (count - ((double)n) / k) * (count - ((double)n) / k) / (((double)n) / k);
		//xi2+=(count-10)*(count-10)/10.0;
	}
...
int main()
{
	...
	constexpr unsigned long beta{ 78'125 };
