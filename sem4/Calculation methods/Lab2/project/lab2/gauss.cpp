...
Matrix Gauss::solveSystem(const Matrix &A, const Matrix &b) const
{
    ...
    for(size_t i{0}; i < A_.getColumnCount()-1; ++i)
    {
        ...
        for(size_t j{i+1}; j < A_.getRowCount(); ++j)
        {
            ...
            if(A.getMatrixElement(j, i) == static_cast<std::complex<double>>(0))
            {
                continue;
            }


            std::complex<double> multiplier{static_cast<std::complex<double>>(-1) * A_.getMatrixElement(j,i) / A_.getMatrixElement(i, i)};
    ...
    // Solve the system itself
    for(int i{(int)A_.getRowCount() - 1}; i >= 0; --i)
    {
        std::complex<double> sum{b_.getMatrixElement(i, 0)};
