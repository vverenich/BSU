...
std::string RSA::decrypt(const std::string& str, double privateKey, int modulus) {
        std::string decryptedMessage;
        for (char c : str) {
            int encryptedChar = static_cast<int>(c);
            double decryptedChar = std::pow(encryptedChar, privateKey);
            decryptedChar = std::fmod(decryptedChar, modulus);
            decryptedMessage += static_cast<char>(decryptedChar);
        }
        return decryptedMessage;
    }

    // Функция для шифрования строки с использованием открытого ключа
    std::string RSA::encrypt(const std::string& str, double publicKey, int modulus) {
        std::string encryptedMessage;
        for (char c : str) {
            int messageChar = static_cast<int>(c);
            double encryptedChar = std::pow(messageChar, publicKey);
            encryptedChar = std::fmod(encryptedChar, modulus);
